...
import dev.chronicles.core.VersionPaths._
...
class VersionPathsSpec extends FlatSpec with Matchers {
  ...
  "Resolving path with a version" should "have the version string appended as a folder" in {
    VersionPaths.pathFor(new URI("s3://foo/bar/"), version) shouldBe new URI(
      s"s3://foo/bar/$VersionColumn=${version.label}")
  }
  ...
  "Parsing the version from versioned paths" should "produce the version number" in {
    VersionPaths.parseVersion(new URI(
      s"file:/tmp/7bbc577c-471d-4ece-8462/table/date=2019-01-21/2019/$VersionColumn=${version.label}")) shouldBe version

    VersionPaths.parseVersion(new URI(s"s3://bucket/pageview/date=2019-01-21/$VersionColumn=${version.label}")) shouldBe version
    VersionPaths.parseVersion(new URI(s"s3://bucket/identity/$VersionColumn=${version.label}")) shouldBe version
  }
  ...
  it should "return strip off the version part of the path" in {
    VersionPaths.versionedToBasePath(new URI(s"hdfs://bucket/identity/$VersionColumn=${version.label}")) shouldBe new URI(
